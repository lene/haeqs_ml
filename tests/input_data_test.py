...
from data_sets.input_data import read_images_from_file
from data_sets.mnist_data_sets import MNISTDataSets

from tests.util import get_project_root_folder
...
class InputDataTest(unittest.TestCase):
    ...
    SAMPLE_MNIST_FILE = get_project_root_folder() + '/tests/data/handwritten_test_data/0.raw'
    SAMPLE_MNIST_FILE_WITH_TWO_IMAGES = get_project_root_folder()+'/tests/data/7_2.raw'
    ...
    def test_read_images_from_file_one(self):
        data = read_images_from_file(
            ...
            MNISTDataSets.IMAGE_SIZE, MNISTDataSets.IMAGE_SIZE, 1
        )
    ...
    def test_read_images_from_file_two(self):
        data = read_images_from_file(
            ...
            MNISTDataSets.IMAGE_SIZE, MNISTDataSets.IMAGE_SIZE, 2
        )
    ...
    def test_read_images_from_file_fails_if_file_too_short(self):
        with self.assertRaises(ValueError):
            read_images_from_file(
                self.SAMPLE_MNIST_FILE_WITH_TWO_IMAGES,
                MNISTDataSets.IMAGE_SIZE, MNISTDataSets.IMAGE_SIZE, 3
            )
    ...
    def test_read_images_from_two_files_using_mnist_data_sets(self):
        data = MNISTDataSets.read_images_from_files(
            get_project_root_folder()+'/tests/data/7_from_test_set.raw',
            get_project_root_folder()+'/tests/data/2_from_test_set.raw'
        )
        self._check_is_n_mnist_images(2, data)
    ...
    def test_read_images_from_three_files_using_mnist_data_sets(self):
        data = MNISTDataSets.read_images_from_files(
            get_project_root_folder()+'/tests/data/7_from_test_set.raw',
            get_project_root_folder()+'/tests/data/2_from_test_set.raw',
            get_project_root_folder()+'/tests/data/1_from_test_set.raw'
        )
    ...
    def _check_is_n_mnist_images(self, n, data):
        ...
        self.assertEqual(MNISTDataSets.IMAGE_SIZE, data.shape[1])
        self.assertEqual(MNISTDataSets.IMAGE_SIZE, data.shape[2])
